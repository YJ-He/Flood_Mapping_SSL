...
class Trainer(object):
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, epochs + 1):
            train_log = self._train_epoch_randmix(epoch)
    ...
    def _train_epoch_randmix(self, epoch):
        ...
        for idx in pbar:
            ...
            if epoch > self.config.warmup_period:
                ...
                # perform cutmix and classmix randomly in each iteration
                if choice == 0:
                    # cutmix
                    batch_mix_masks = torch.from_numpy(mask_generator.generate_params(unsup_imgs_0.shape[0], (
                        self.config.input_size, self.config.input_size)).astype(dtype=np.float32)).to(self.device,
                                                                                                      non_blocking=True)
                    if not self.config.use_mix:
                        batch_mix_masks = torch.zeros_like(batch_mix_masks)
                    ...
                    prob_cons_tea_1[prob_cons_tea_1 < 0.5] = 0
                    prob_cons_tea_1[prob_cons_tea_1 >= 0.5] = 1
                    ps_label_1 = torch.squeeze(prob_cons_tea_1, dim=1)

                    prob_cons_tea_2[prob_cons_tea_2 < 0.5] = 0
                    prob_cons_tea_2[prob_cons_tea_2 >= 0.5] = 1
                    ps_label_2 = torch.squeeze(prob_cons_tea_2, dim=1)
                    ...
                    with torch.no_grad():
                        ...
                        prob_u1_tea_1_temp = torch.zeros_like(prob_u1_tea_1)
                        prob_u1_tea_1_temp[prob_u1_tea_1 < 0.5] = 0
                        prob_u1_tea_1_temp[prob_u1_tea_1 >= 0.5] = 1
                        ps_u1_tea_label_1 = torch.squeeze(prob_u1_tea_1_temp, dim=1)
            ...
            with autocast():
                if epoch > self.config.warmup_period:  # warmup
                    ...
                    logits_cons_stu_1 = torch.squeeze(logits_cons_stu_1, 1)
                    logits_cons_stu_2 = torch.squeeze(logits_cons_stu_2, 1)
                ...
                sup_prob_l = torch.sigmoid(sup_logits_l)

                sup_logits_r = torch.squeeze(sup_logits_r, 1)
                sup_prob_r = torch.sigmoid(sup_logits_r)
